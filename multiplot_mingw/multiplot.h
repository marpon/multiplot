...
void msg_exit(std::string s1 )
{
    MessageBoxA(0, s1.c_str() , "Error message, will exit now!", MB_OK | MB_ICONERROR | MB_TOPMOST);
    exit(1);
}
...
protected:
    ...
    */
    bool CreateGLWindow(int x, int y, int width, int height, const std::wstring& title, BYTE bits=0, bool fullscreenflag=false)
    {
        ...
        {
            //throw std::exception();
            msg_exit("Failed To Register The Window Class.");
        }
        ...
        if (!(hWnd=CreateWindowEx(	dwExStyle,	// Extended Style For The Window
                                    "OpenGL",							// Class Name
                                    (LPCSTR)title_str.c_str(),							// Window Title
                                    ...
                                    this)))								// Dont Pass Anything To WM_CREATE
        {
            ...
            msg_exit("Window Creation Error.");
        }
        ...
        if (!(hDC=GetDC(hWnd)))							// Did We Get A Device Context?
        {
            ...
            msg_exit("Can't Create A GL Device Context.");
        }
        ...
        if (!(PixelFormat=ChoosePixelFormat(hDC,&pfd)))	// Did Windows Find A Matching Pixel Format?
        {
            ...
            msg_exit("Can't Find A Suitable PixelFormat.");
        }
    ...
    void DestroyGLWindow()
    {
        ...
        if (hRC)											// Do We Have A Rendering Context?
        {
            if (!wglMakeCurrent(NULL,NULL))					// Are We Able To Release The DC And RC Contexts?
            {
                ...
                msg_exit("Release Of DC And RC Failed.");
            }

            if (!wglDeleteContext(hRC))						// Are We Able To Delete The RC?
            {
                //throw(std::exception("Release Rendering Context Failed."));
                msg_exit("Release Rendering Context Failed.");
            }
        ...
        if (hDC && !ReleaseDC(hWnd,hDC))					// Are We Able To Release The DC
        {
            ...
            msg_exit("Release Device Context Failed.");
        ...
        if (hWnd && !DestroyWindow(hWnd))					// Are We Able To Destroy The Window?
        {
            ...
            msg_exit("Could Not Release hWnd.");
        ...
        if (!UnregisterClass("OpenGL",hInstance))			// Are We Able To Unregister Class
        {
            ...
            msg_exit("Could Not Unregister Class.");
...
public:
    ...
    */
    ...
    {
        title_str = t;
        SetWindowTextW(hWnd, title_str.c_str());
    }
...
public:
    ...
    */
    ...
    public:
        ...
        void draw(Point2d& minimum, Point2d& maximum, const Point2d& scale, const Point2d& offset)
        {
            ...
            for(size_t a = start; a < start + size() - 1; a++)
            {
                if ((a + 1) == pos)
                {
                    continue;    // in MP_SCROLL_WARP mode, skip this line
                }
    ...
    public:
        ...
        */
        ...
        {
            cur_point_size = psize;
            if(psize >= 1.0 )
                cur_point_size = psize + 0.1;
            else
                cur_point_size = 0.0;
        }
        ...
        */
        ...
        {
            if (MP_NO_SCROLL != scrolling_type_ && number_of_points_to_plot_ <= 0)
            {
                throw("error calling Multiplot::scrolling(): The number of points to plot must be a positive integer greater zero.");
            }
    ...
    */
    template<class T> void plot(const std::vector<T>& vx, const std::vector<T>& vy)
    {
        if (vx.size() != vy.size())
        {
            throw std::length_error("Multiplot: both vectors must have the same length.\n");
        }
    ...
    */
    void color3f(float r, float g, float b)
    {
        traces[cur_trace].color3f(r,g,b);
    }
    ...
    */
    ...
    {
        title_str = std::wstring(title_.begin(), title_.end());
    }
    ...
    */
    void linewidth(float width)
    {
        traces[cur_trace].linewidth(width);
    }
    /**
    ...
    */
    void pointsize(float psize)
    {
        traces[cur_trace].pointsize(psize);
    }
    /**
    * changes scrolling behaviour for current trace - see class Trace for details.
    */
    void scrolling(MP_SCROLLING scrolling_type, int max_points_to_plot=-1)
    {
        traces[cur_trace].scrolling(scrolling_type, max_points_to_plot);
    }

    // short hand for backwards compatibility with older Multiplot versions
    void scrolling(int max_points_to_plot)
    {
        traces[cur_trace].scrolling(MP_SCROLL_LEFT, max_points_to_plot);
    }
...
protected:
    ...
    virtual void draw()
    {
        ...
        case MP_AUTO_SCALE:
            if (diff_x != 0.0)
            {
                scale.x = width / diff_x;
            }
            if (diff_y != 0.0)
            {
                scale.y = height / diff_y;
            }
            break;
...
void demo9()
{
    std::string s1 = "More than one Multiplot window is currently only supported with FLTK as a backend.\n";
#ifndef MULTIPLOT_FLTK
    cerr << s1;
    MessageBoxA(0, s1.c_str() , "Warning message.", MB_OK | MB_ICONWARNING | MB_TOPMOST);
    return;
#endif
